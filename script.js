...
window.addEventListener("load", function () {
    ...
    // 自动刷新文件列表
    document.querySelector("#settings").addEventListener("closed", function () {
        refreshFileList();
    });
    // 回车键操作
        ...
        switch (option) {
            case "delete":
                ...
                if (isFinalStep) {
                    ...
                    document.querySelector("#multipleOperationStatus").innerText = "已删除" + multipleSelected.length + "个项目";
                    document.querySelector("#multipleOperationStatus").open = true;
                    refreshFileList();
                } else {
